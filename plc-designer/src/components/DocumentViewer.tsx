...
export function DocumentViewer() {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    accept: { 'application/pdf': ['.pdf'], 'image/*': ['.jpg', '.jpeg', '.png', '.svg'] },
  });
    ...
    <div className="flex h-full gap-4">
      <div className="w-48 flex flex-col gap-2">
        <div {...getRootProps()}
          ...
          <p className="text-xs text-slate-500">Drop PDF/image or click</p>
        </div>
        ...
        <div className="flex-1 overflow-auto space-y-1">
          {documents.map((doc) => (
            <div key={doc.id}
              ...
              {doc.type === 'pdf' ? <FileText size={14} className="text-red-400 flex-shrink-0" /> : <Image size={14} className="text-blue-400 flex-shrink-0" />}
